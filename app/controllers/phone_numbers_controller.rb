class PhoneNumbersController < ApplicationController
  ...
  def index
    ...
    @phone_numbers = @phone_numbers.where(:branch_office_id => params[:branch_office_id]) unless params[:branch_office_id].blank?

    @phone_numbers = @phone_numbers.where(["phone_number LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?

    @phone_numbers = @phone_numbers.order(:phone_number).page(params[:page] || 1)
  end
  ...
  private
    ...
    def phone_number_params
      params.require(:phone_number).permit(:phone_number, :branch_office_id)
    end
end
