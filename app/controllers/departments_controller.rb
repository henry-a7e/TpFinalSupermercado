class DepartmentsController < ApplicationController
  ...
  def index
    ...
    @departments = @departments.where(["department_name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?

    @departments = @departments.order(:department_name).page(params[:page] || 1)
  end
